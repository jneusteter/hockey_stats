HockeyStatsTracker::Admin.controllers :teams do
  ...
  get :edit, with: :id do
    ...
    if @team
      ...
      flash[:warning] = pat(:create_error, model: 'team', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  put :update, with: :id do
    ...
    if @team
      if @team.modified! && @team.update(params[:team])
        flash[:success] = pat(:update_success, model: 'Team', id: params[:id].to_s)
      ...
      flash[:warning] = pat(:update_warning, model: 'team', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy, with: :id do
    ...
    if team
      if team.destroy
        flash[:success] = pat(:delete_success, model: 'Team', id: params[:id].to_s)
      else
        flash[:error] = pat(:delete_error, model: 'team')
      end
      redirect url(:teams, :index)
    else
      flash[:warning] = pat(:delete_warning, model: 'team', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy_many do
    ...
    if teams.destroy
      ...
      flash[:success] = pat(:destroy_many_success, model: 'Teams', ids: ids.join(', ').to_s)
    end
    redirect url(:teams, :index)
  end
end
