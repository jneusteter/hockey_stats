HockeyStatsTracker::Admin.controllers :goals do
  ...
  get :edit, with: :id do
    ...
    if @goal
      ...
      flash[:warning] = pat(:create_error, model: 'goal', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  put :update, with: :id do
    ...
    if @goal
      if @goal.modified! && @goal.update(params[:goal])
        flash[:success] = pat(:update_success, model: 'Goal', id: params[:id].to_s)
      ...
      flash[:warning] = pat(:update_warning, model: 'goal', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy, with: :id do
    ...
    if goal
      if goal.destroy
        flash[:success] = pat(:delete_success, model: 'Goal', id: params[:id].to_s)
      else
        flash[:error] = pat(:delete_error, model: 'goal')
      end
      redirect url(:goals, :index)
    else
      flash[:warning] = pat(:delete_warning, model: 'goal', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy_many do
    ...
    if goals.destroy
      ...
      flash[:success] = pat(:destroy_many_success, model: 'Goals', ids: ids.join(', ').to_s)
    end
    redirect url(:goals, :index)
  end
end
