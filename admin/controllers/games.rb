HockeyStatsTracker::Admin.controllers :games do
  get :index do
    @title = 'Games'
    @games = Game.all
    render 'games/index'
  ...
  get :edit, with: :id do
    ...
    if @game
      ...
      flash[:warning] = pat(:create_error, model: 'game', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  put :update, with: :id do
    ...
    @game = Game[params[:id]]
    if @game
      if @game.modified! && @game.update(params[:game])
        flash[:success] = pat(:update_success, model: 'Game', id: params[:id].to_s)
      ...
      flash[:warning] = pat(:update_warning, model: 'game', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy, with: :id do
    ...
    if game
      if game.destroy
        flash[:success] = pat(:delete_success, model: 'Game', id: params[:id].to_s)
      else
        flash[:error] = pat(:delete_error, model: 'game')
      end
      redirect url(:games, :index)
    else
      flash[:warning] = pat(:delete_warning, model: 'game', id: params[:id].to_s)
      halt 404
    end
  end
  ...
  delete :destroy_many do
    ...
    if games.destroy
      ...
      flash[:success] = pat(:destroy_many_success, model: 'Games', ids: ids.join(', ').to_s)
    end
    redirect url(:games, :index)
  end
end
